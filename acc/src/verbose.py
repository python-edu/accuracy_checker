...
class Verbose:
        ...
        if self.verbose:
            if self._is_args_data(data):
                ...
                if self._check_df_len(data) <= 160:
                    ...
                    table = tabulate(data, headers="keys", showindex=True, tablefmt="pretty")
    ...
    def _format_line(self, line: str, width: int, subsequent_indent: str = " ") -> str:
        """
        ...
        Args:
            ...
            subsequent_indent (str): The string to use for indenting subsequent lines.
    ...
    def _format_dict(self, dc: dict, width: int, char: str = " ") -> list[str]:
        ...
        Returns:
            list[str]: A list of formatted strings, one for each key-value pair.
        """
        ...
        for key, val in dc.items():
            ...
            res.append("\n".join(textwrap.wrap(line, width=width, subsequent_indent=(k + 6) * " ")))
        return res
    ...
    def _check_df_len(self, df: pd.DataFrame) -> int:
        """
        Checks whether a DataFrame is small enough to be displayed in its entirety.
    ...
    def _print_args(self, args: Any) -> str:
        ...
        for key in keys_order:
            if key in args_dict:
                formatted_args.append(f"{key: >{max_len}}:   {self._format_line(args_dict[key], width=100, subsequent_indent=3 * ' ')}")

        # Handle report_data separately
        if "report_data" in args_dict:
            formatted_args.append("\n report_data:")
            for key, val in args_dict["report_data"].items():
                formatted_args.append(f"{key: >{max_len}}:   {self._format_line(val, width=100)}")
    ...
    def _is_args_data(self, data: Any) -> bool:
        ...
        Returns:
            bool: True if the data is an argparse.Namespace instance, else False.
        """
