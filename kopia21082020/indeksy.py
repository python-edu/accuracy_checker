...
Klasy:
  ## 1. 'accClasic'    - oblicza wskażniki 'klasyczne' na podstawie 'cross matrix'
  ## 2. 'accClasicBin' - oblicza wskażniki 'klasyczne' na podstawie 'binTF'
  ## 2. 'accIndex'     - oblicza wskażniki z mchine learning na podstawie 'binTF'
...
class accClasic:
    ...
    def _obliczIndeksy(self):
        self.OA = self._overallAccuracy()
        self.PA = self._producerAcc()
        self.UA = self._userAcc()
        
        self.OME = self._errorsOfOmission()
        self.CME = self._errorsOfCommission()
...
if __name__ == '__main__':
    ...
    # wynik referencyjny (poprawny) dla testów tradycyjnych indeksów dokładności
    tabelaRef = pd.DataFrame(\
        {'OA':[0.7789474,0.7789474,0.7789474], 'PA': [0.63636364, 0.79487179, 0.95652174],\
         'UA':[0.77777778, 0.83783784, 0.70967742], 'OME':[0.36363636, 0.20512821, 0.04347826],\
         'CME':[0.22222222, 0.16216216, 0.29032258]},index=['water','forest','urban'])
    
    
    # inicjacja instancji klasy
    cl = accClasic(cros,prec)
    # uzgodnienie precyzji - zaokrąglenie
    tabelaRef = np.round(tabelaRef,cl.precision)
    
    assert tabelaRef.equals(cl.tabela), f'''\n\nBłędy klasy 'accClasic'!!!!!': tabelaRef:\n{tabelaRef}\n\ncl.tabela:\n{cl.tabela}\n'''
    
    #w = int(round(0.7789474,cl.precision) * 10**cl.precision)
    #t = int(cl.OA * 10**cl.precision)
    #assert t == w, f'''\n\nBłąd klasy 'accClasic', metody 'OA': {t} != {w}\n'''
    
    #w = np.round(np.array([0.63636364, 0.79487179, 0.95652174]), cl.precision)
    #t = cl.PA.to_numpy()
    #assert np.all(t==w), f'''\n\nBłąd klasy 'accClasic', metody 'PA'.\n'''
    
    #w = np.round(np.array([0.77777778, 0.83783784, 0.70967742]), cl.precision)
    #t = cl.UA.to_numpy()
    #assert np.all(t==w), f'''\n\nBłąd klasy 'accClasic', metody 'UA'.\n'''   

    #w = np.round(np.array([0.36363636, 0.20512821, 0.04347826]), cl.precision)
    #t = cl.OME.to_numpy()
    #assert np.all(t==w), f'''\n\nBłąd klasy 'accClasic', metody 'OME'.\n'''
    
    #w = np.round(np.array([0.22222222, 0.16216216, 0.29032258]), cl.precision)
    #t = cl.CME.to_numpy()
    #assert np.all(t==w), f'''\n\nBłąd klasy 'accClasic', metody 'CME'.\n'''    

    print('''\n......... Wszystkie testy klasy 'accClasic' pozytywne - OK!\n\n''')

    #==============================================================================
    # 2. Test klasy 'accIndex'
    #    - dane testowe - binTF
 ...
 # prawidłowe odpowiedzi 'indeksów' modern2
    ...
    #==============================================================================
    # 3. Test klasy 'accClasicBin'
    #    - dane testowe - binTF - takie same jak dla 'accIndex'
    # tworzy instancję klasy
    
    cl = accClasicBin(binTF,prec)
    
    print(f'\n\n\nTest klasy accClasicBin!\n')
    #for k,v in sorted(vars(cl).items()):
        #print(f'{k}:\n{v}\n\n')
    
    assert tabelaRef.equals(cl.tabela), f'''\n\nBłędy klasy 'accClasicBin'!!!!!': tabelaRef:\n{tabelaRef}\n\ncl.tabela:\n{cl.tabela}\n'''
